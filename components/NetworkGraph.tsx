...
export default function NetworkGraph({ window }: { window: number }) {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            fetch(
                `http://localhost:8080/api/correlations?window=${window}`,
            ).then((r) => r.json()),
        ]).then(([a, c]) => {
            setAssets(a);
            setCorrelations(c);
    ...
    useEffect(() => {
        ...
        const link = svg
            ...
            .on("mouseleave", function (event, d) {
                ...
                d3.select(this)
                    .attr(
                        "stroke-opacity",
                        edgeOpacity(d.correlation, d.zscore),
                    )
        ...
        // anomaly glow
        node.filter((d) => anomalousNodes.has(d.id))
            .append("circle")
            .attr("r", 24)
            .attr("fill", "none")
            .attr("stroke", "#ef4444")
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500">
                    ...
                    <div className="flex items-center gap-1 ml-2">
                        <div className="w-4 border-t-2 border-green-500" />
                        <span>positive</span>
                    </div>
                    <div className="flex items-center gap-1">
                        <div className="w-4 border-t-2 border-red-500" />
                        <span>negative</span>
                    </div>
                </div>
            </div>
            ...
            <div className="h-5">
                {hoveredPair && (
                    <p className="text-xs font-mono text-gray-600 bg-gray-50 px-2 py-1 rounded inline-block">
                        {hoveredPair}
                    </p>
                )}
            </div>
            ...
            <p className="text-xs text-gray-400">
                Dashed edges = anomalous (|z| &gt; 2σ). Red ring = asset
                involved in anomaly. Drag nodes to explore. Adjust threshold to
                show stronger correlations only.
            </p>
        </div>
    );
}
